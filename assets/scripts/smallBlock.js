...
let changeposition = 0 //用于计算在数组中的位置
let changeangle = 0 //用于计算角度
  ...
  onLoad() {
    ...
    this.node.on(cc.Node.EventType.TOUCH_END, function (touch, event) {
      if (this.hit && this.node.blocktype == 'small') {
        ...
        // 记录当前放置的位置
        let currentindex = -1;
        const points = global.insidepoints
        ...
        if (this.node.x * this.node.x + this.node.y * this.node.y > 474 * 474 / 4) {
          ...
          // 在对应圆环已放置的数组中查找
          global.insideblocks.forEach(item => {
          ...
          if (isempty && this.node.x != 0 && this.node.y != 0) {
            ...
            //计算旋转角度
            global.Schangeangle += 45 * parseInt(this.game.rotateGridsS.string)

            //计算块的真正位置
            let n = (currentindex + global.Schangeposition) % 8
            if (n < 0)
              n += 8
            ...
            cc.tween(this.node).to(.2, {
              ...
              this.game.inside.addChild(this.node);

              //环旋转的缓动效果
              cc.tween(this.game.inside).to(Math.abs(parseInt(this.game.rotateGridsS.string)) / 4, {
            ...
            let block = {
              ...
              rotateGridsS: this.game.rotateGridsS.string, // 块的转动格数
              blockitem: this
            }

            // 保存在圆环对象数组中
            global.insideblocks.push(block)

            // 播放音效
            cc.audioEngine.playEffect(this.game.placeAudio, false);

            //计算在数组中的位置
            global.Schangeposition -= parseInt(this.game.rotateGridsS.string)

            //更新数组中所有块的位置
            global.insideblocks.forEach(item => {
              item.index = (item.index + parseInt(this.game.rotateGridsS.string)) % 8
              if (item.index < 0)
                item.index += 8
            })
