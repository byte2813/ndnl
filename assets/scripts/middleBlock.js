  ...
  onLoad() {
    ...
    this.node.on(cc.Node.EventType.TOUCH_END, function (touch, event) {
      if (this.hit && this.node.blocktype == 'middle') {
        ...
        // 记录当前放置的位置
        let currentindex = -1;
        const points = global.besidepoints

        // 没有移动到圆环目标位置区域，移动到初始位置
        if (this.node.x * this.node.x + this.node.y * this.node.y > 774 * 774 / 4 || this.node.x * this.node.x +
          this.node.y * this.node.y < 274 * 274 / 4) {
          ...
          // 在对应圆环已放置的数组中查找
          global.besideblocks.forEach(item => {
          ...
          if (isempty && this.node.x != 0 && this.node.y != 0) {
            ...
            //计算旋转角度
            global.Mchangeangle += 45 * parseInt(this.game.rotateGridsM.string)

            //计算块的真正位置
            let n = (currentindex + global.Mchangeposition) % 8
            if (n < 0)
              n += 8
            ...
            cc.tween(this.node).to(.2, {
              ...
              this.game.beside.addChild(this.node);

              //环旋转的缓动效果
              cc.tween(this.game.beside).to(Math.abs(parseInt(this.game.rotateGridsM.string)) / 4, {
            ...
            let block = {
              ...
              rotateGridsM: this.game.rotateGridsM.string, // 块的转动格数
              blockitem: this
            }

            // 保存在圆环对象数组中
            global.besideblocks.push(block)

            // 播放音效
            cc.audioEngine.playEffect(this.game.placeAudio, false);

            //计算在数组中的位置
            global.Mchangeposition -= parseInt(this.game.rotateGridsM.string)

            //更新数组中所有块的位置
            global.besideblocks.forEach(item => {
              item.index = (item.index + parseInt(this.game.rotateGridsM.string)) % 8
              if (item.index < 0)
                item.index += 8
            })
